...
def grid2values(grid):
    ...
    """
    sudoku_grid = {}
    for val, key in zip(grid, boxes_all):
        if val == '.':
...
def display(values):
    ...
    """
    width = 1 + max(len(values[s]) for s in boxes_all)
    line = '+'.join(['-' * (width * 3)] * 3)
    for r in rows:
...
def reconstruct(values, hist):
    ...
    hist(dict)
        a history dictionary of the form {key: (key, (box, value))} encoding a linked
